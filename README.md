...
Il progetto include il workflow `.github/workflows/build-windows.yml` per creare automaticamente la versione Windows (`flutter build windows --release`). Il workflow genera il runner Windows, usa SQLite FFI e pubblica `Preventivi-Windows.zip` come artifact di GitHub Actions.
