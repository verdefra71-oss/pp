...
''');
  ...
  Future<int> updatePreventivoAcconti({
    required int id,
    required List<Map<String, dynamic>> acconti,
  }) async {
    final result = await (await database).update(
      'preventivi',
      {'acconti': jsonEncode(acconti), 'numero_rate': acconti.length},
      where: 'id = ?',
      whereArgs: [id],
    );
    final db = await database;
    await db.delete('rate', where: 'preventivo_id = ?', whereArgs: [id]);
    final p = await db.query('preventivi', where: 'id = ?', whereArgs: [id], limit: 1);
    final cliente = p.isNotEmpty ? (p.first['cliente'] ?? '').toString() : '';
    for (final a in acconti) {
      final importo = (a['importo'] as num?)?.toDouble() ?? 0;
      final data = DateTime.tryParse((a['data'] ?? '').toString());
      if (importo > 0 && data != null) {
        await db.insert('rate', {
          'preventivo_id': id,
          'cliente': cliente,
          'importo': importo,
          'data_scadenza': data.toIso8601String(),
          'pagata': (a['pagata'] == true || a['pagata'] == 1) ? 1 : 0,
        });
      }
    }
    await autoBackup();
...
class PdfGenerator {
  static Future<void> generaECondividiPreventivo({
    ...
    final totalePagato = acconti.fold<double>(0, (sum, a) => sum + ((a['pagata'] == true || a['pagata'] == 1) ? ((a['importo'] as num?)?.toDouble() ?? 0) : 0));
    final saldo = (totale - totalePagato).clamp(0, double.infinity).toDouble();
    final documentoPagato = saldo <= 0.01;
          ...
          pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                pw.Text(
                  documentoPagato ? 'PAGATO' : 'Saldo da pagare: € ${saldo.toStringAsFixed(2)}',
                  style: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: documentoPagato ? PdfColors.green : dark),
                ),
              ],
            ),
          ),
                    ...
                    pw.Container(
                      ...
                      child: pw.Row(
                        ...
                        children: [
                          ...
                          pw.Text((acconti[i]['pagata'] == true || acconti[i]['pagata'] == 1) ? 'PAGATO' : 'DA PAGARE'),
                        ],
                      ),
                    ),
                ],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        backgroundColor: const Color(0xFFE8C65A),
          ...
          children: [
            ...
            Container(
              ...
              child: const Align(
                ...
                child: Text('Gestione', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color(0xFF4A3500))),
              ),
            ),
            ...
            const Text('Riepilogo', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w800, color: Color(0xFF705300))),
      ...
      child: Container(
        ...
        child: Column(children: [
          ...
          const Text('Gestione Preventivi', textAlign: TextAlign.center,
            style: TextStyle(color: Color(0xFF4A3500), fontSize: 26, fontWeight: FontWeight.w900)),
          const SizedBox(height: 5),
          const Text('Crea, salva e condividi i tuoi preventivi.', textAlign: TextAlign.center,
            style: TextStyle(color: Color(0xFF6B5A2A), fontSize: 14)),
          const SizedBox(height: 18),
          SizedBox(
      ...
      child: Card(
        ...
        child: Padding(
          ...
          child: Column(children: [
            Container(
              width: 44, height: 44,
              decoration: BoxDecoration(color: const Color(0xFFFFE8A3), borderRadius: BorderRadius.circular(14)),
              child: Icon(icon, size: 32, color: const Color(0xFF8A6500)),
            ),
            const SizedBox(height: 4),
            Text(loading ? '…' : '$value',
              style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w900, color: Color(0xFF3D2E00))),
            Text(label, style: const TextStyle(color: Color(0xFF6B5A2A))),
          ]),
        ),
      ),
    );
        ...
        child: Padding(
          ...
          child: ListTile(
            leading: Container(
              width: 54, height: 54,
              decoration: BoxDecoration(color: const Color(0xFFFFE8A3), borderRadius: BorderRadius.circular(14)),
              child: Icon(icon, size: 28, color: const Color(0xFF8A6500)),
            ),
            title: Text(title, style: const TextStyle(fontWeight: FontWeight.w700)),
            subtitle: Text(subtitle),
            trailing: const Icon(Icons.arrow_forward_ios_rounded, size: 16, color: Color(0xFF8A6500)),
          ),
        ),
      ),
    );
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Preventivo $numero salvato correttamente.',
            ),
          ),
        );
          ...
          children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          Checkbox(
                            value: a['pagata'] == true || a['pagata'] == 1,
                            onChanged: (v) => setState(() => a['pagata'] = v ?? false),
                          ),
                          const Text('Pagato', style: TextStyle(fontSize: 11)),
                        ],
                      ),
                      IconButton(
            ...
            if (acconti.isNotEmpty) ...[
              Builder(builder: (_) {
                ...
                final sommaPagata = acconti.fold<double>(
                  0, (sum, a) => sum + ((a['pagata'] == true || a['pagata'] == 1) ? ((a['importo'] as num?)?.toDouble() ?? 0) : 0),
                );
                final residuo = (totale - sommaPagata).clamp(0, double.infinity).toDouble();
                return Padding(
                  padding: const EdgeInsets.only(top: 4, bottom: 8),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: [
                      Text('Totale acconti programmati: € ${sommaAcconti.toStringAsFixed(2)}'),
                      Text('Pagato: € ${sommaPagata.toStringAsFixed(2)}'),
                      Text(
                        residuo <= 0.01 ? 'PAGATO' : 'Saldo da pagare: € ${residuo.toStringAsFixed(2)}',
                        style: TextStyle(
                          fontWeight: FontWeight.bold,
...
class _ListaPreventiviScreenState extends State<ListaPreventiviScreen> {
  ...
  String _statoPagamento(Map<String, dynamic> x) {
    final totale = (x['totale'] as num?)?.toDouble() ?? 0;
    final acconti = _accontiDaPreventivo(x);
    final pagato = acconti.fold<double>(0, (s, a) => s + ((a['pagata'] == true || a['pagata'] == 1) ? ((a['importo'] as num?)?.toDouble() ?? 0) : 0));
    final residuo = (totale - pagato).clamp(0, double.infinity).toDouble();
    if (residuo <= 0.01 && totale > 0) return 'PAGATO';
    if (pagato > 0) return 'PAGAMENTO PARZIALE • Saldo € ${residuo.toStringAsFixed(2)}';
    return 'DA PAGARE • € ${residuo.toStringAsFixed(2)}';
  }
...
class AccontiScreen extends StatefulWidget {
  const AccontiScreen({super.key});

  @override
  State<AccontiScreen> createState() => _AccontiScreenState();
}

class _AccontiScreenState extends State<AccontiScreen> {
  Future<List<Map<String, dynamic>>> _carica() async {
    ...
    for (final p in preventivi) {
      final accettato = (p['accettato'] as num?)?.toInt() == 1;
      if (!accettato) continue;
      try {
        final raw = jsonDecode((p['acconti'] ?? '[]').toString());
        if (raw is List) {
          for (var i = 0; i < raw.length; i++) {
            ...
            result.add({
              ...a,
              'preventivo_id': (p['id'] as num).toInt(),
              'cliente': p['cliente'],
              'numero': p['numero'],
              'totale': (p['totale'] as num?)?.toDouble() ?? 0,
              'acconto_index': i,
            });
          }
        }
  ...
  Future<void> _togglePagato(Map<String, dynamic> x, bool value) async {
    final id = x['preventivo_id'] as int;
    final preventivi = await DatabaseHelper.instance.getPreventivi();
    final p = preventivi.firstWhere((e) => (e['id'] as num).toInt() == id);
    final raw = jsonDecode((p['acconti'] ?? '[]').toString());
    final acconti = raw is List
        ? raw.map((e) => Map<String, dynamic>.from(e as Map)).toList()
        : <Map<String, dynamic>>[];
    final i = x['acconto_index'] as int;
    if (i < 0 || i >= acconti.length) return;
    acconti[i]['pagata'] = value;
    await DatabaseHelper.instance.updatePreventivoAcconti(id: id, acconti: acconti);
    if (mounted) setState(() {});
  }
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (acconti.isEmpty) {
            return const _EmptyState(icon: Icons.payments_outlined, text: 'Nessun acconto di preventivo accettato.');
          }
          final gruppi = <int, List<Map<String, dynamic>>>{};
          for (final x in acconti) {
            gruppi.putIfAbsent(x['preventivo_id'] as int, () => []).add(x);
          }
          return ListView(
            padding: const EdgeInsets.all(12),
            children: [
              for (final entry in gruppi.entries) _gruppoPreventivo(entry.value),
            ],
          );
        },
      ),
  ...
  Widget _gruppoPreventivo(List<Map<String, dynamic>> items) {
    final totale = (items.first['totale'] as num?)?.toDouble() ?? 0;
    final pagato = items.fold<double>(0, (s, a) => s + ((a['pagata'] == true || a['pagata'] == 1) ? ((a['importo'] as num?)?.toDouble() ?? 0) : 0));
    final residuo = (totale - pagato).clamp(0, double.infinity).toDouble();
    final completamentePagato = residuo <= 0.01;
    return Card(
      margin: const EdgeInsets.only(bottom: 12),
      child: Padding(
        padding: const EdgeInsets.all(12),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('${items.first['numero']} • ${items.first['cliente']}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 5),
          Text('Totale: € ${totale.toStringAsFixed(2)}   •   Pagato: € ${pagato.toStringAsFixed(2)}'),
          Text(completamentePagato ? 'PAGATO' : 'Saldo da pagare: € ${residuo.toStringAsFixed(2)}', style: TextStyle(fontWeight: FontWeight.bold, color: completamentePagato ? Colors.green : Theme.of(context).colorScheme.primary)),
          const Divider(height: 18),
          for (var i = 0; i < items.length; i++) _rigaAcconto(items[i], i),
        ]),
      ),
    );
  }

  Widget _rigaAcconto(Map<String, dynamic> x, int index) {
    final data = DateTime.tryParse((x['data'] ?? '').toString()) ?? DateTime.now();
    final pagato = x['pagata'] == true || x['pagata'] == 1;
    final importo = (x['importo'] as num?)?.toDouble() ?? 0;
    return Row(children: [
      Checkbox(value: pagato, onChanged: (v) => _togglePagato(x, v ?? false)),
      Expanded(child: Text('Acconto ${index + 1} • € ${importo.toStringAsFixed(2)}\n${DateFormat('dd/MM/yyyy').format(data)}${pagato ? ' • PAGATO' : ' • DA PAGARE'}')),
    ]);
  }
}
